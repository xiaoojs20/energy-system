...
class MplCanvas(FigureCanvasQTAgg):
    ...
    def __init__(self, parent=None, width=5, height=4, dpi=100):
        ...
        fig.suptitle("PV Prediction")
        self.axes = fig.add_subplot(111)
        super(MplCanvas, self).__init__(fig)

class Pred_Window(QtWidgets.QMainWindow):

    def __init__(self, *args, **kwargs):
        super(Pred_Window, self).__init__(*args, **kwargs)
        ...
        df_pred = pd.read_csv('data/pred.csv', header=0, infer_datetime_format=True, parse_dates=['index_test'], index_col=['index_test'])


        # plot the pandas DataFrame, passing in the
        # matplotlib Canvas axes.
        # df.plot(ax=sc.axes)
        df_test.plot(ax=sc.axes)
        df_pred.plot(ax=sc.axes)
    ...
    def initUI(self):
        ...
        self.setWindowTitle("PV Prediction")
        self.setWindowIcon(QIcon('resource/太阳_sun.png'))
        self.show()
